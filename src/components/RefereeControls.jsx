...
const RefereeControls = ({ onPointAwarded, onClose }) => {
    ...
    const winnerName = winner === 'A' ? 'KARASUNO' : 'NEKOMA';

    const headerColor = winner === 'A' ? 'bg-orange-600' : 'bg-red-600';
            ...
            <div className="bg-slate-900 w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col border border-slate-700 h-[80vh]">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div className="text-white/90 font-bold text-xs uppercase tracking-widest mt-1">
                                {isConfirming ? `${state.currentCall.fault.name} • ${state.currentCall.faultingTeam} FAULT` : 'SELECT INFRACTION'}
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-hidden flex relative">
                    ...
                    <div className="flex-1 bg-slate-100 flex flex-col border-r border-slate-300">
                        <div className="bg-orange-100 p-3 text-center border-b border-orange-200 shadow-sm z-10"><span className="text-orange-800 font-black text-sm uppercase tracking-widest">KARASUNO FAULT</span></div>
                        <div className="flex-1 overflow-y-auto p-3 grid grid-cols-2 gap-3 content-start bg-slate-50">
                            {Object.values(FAULTS).map(f => (
                                <button key={f.code} onClick={() => handleFault(f.code, 'A')} className="bg-white border-2 border-slate-200 hover:border-orange-500 hover:bg-orange-50 text-slate-600 hover:text-orange-700 rounded-xl p-4 flex flex-col items-center justify-center gap-2 transition-all active:scale-95 shadow-sm h-28 group">
                    ...
                    <div className="flex-1 bg-slate-100 flex flex-col border-l border-slate-300">
                        <div className="bg-red-100 p-3 text-center border-b border-red-200 shadow-sm z-10"><span className="text-red-800 font-black text-sm uppercase tracking-widest">NEKOMA FAULT</span></div>
                        <div className="flex-1 overflow-y-auto p-3 grid grid-cols-2 gap-3 content-start bg-slate-50">
                            {Object.values(FAULTS).map(f => (
                                <button key={f.code} onClick={() => handleFault(f.code, 'B')} className="bg-white border-2 border-slate-200 hover:border-red-500 hover:bg-red-50 text-slate-600 hover:text-red-700 rounded-xl p-4 flex flex-col items-center justify-center gap-2 transition-all active:scale-95 shadow-sm h-28 group">
