...
// --- DEFAULT DATA (UPDATED TO MATCH SETUP PAGE STRUCTURE) ---
const INITIAL_ROSTERS = {
    home: {
        court: [
            { id: 'k1', number: 9, name: 'KAGEYAMA', pos: 'S', isLibero: false },
            { id: 'k2', number: 5, name: 'TANAKA', pos: 'OH', isLibero: false },
            { id: 'k3', number: 10, name: 'HINATA', pos: 'MB', isLibero: false },
            { id: 'k4', number: 3, name: 'ASAHI', pos: 'OH', isLibero: false },
            { id: 'k5', number: 11, name: 'TSUKISHIMA', pos: 'MB', isLibero: false },
            { id: 'k6', number: 1, name: 'DAICHI', pos: 'OPP', isLibero: false }
        ],
        liberos: [{ id: 'kL1', number: 4, name: 'NISHINOYA', pos: 'L', isLibero: true }],
        bench: [
            { id: 'kb1', number: 2, name: 'SUGAWARA', pos: 'S', isLibero: false },
            { id: 'kb2', number: 12, name: 'YAMAGUCHI', pos: 'MB', isLibero: false },
            { id: 'kb3', number: 6, name: 'ENNOSHITA', pos: 'OH', isLibero: false }
        ]
    },
    away: {
        court: [
            { id: 'n1', number: 1, name: 'KUROO', pos: 'MB', isLibero: false },
            { id: 'n2', number: 5, name: 'KENMA', pos: 'S', isLibero: false },
            { id: 'n3', number: 11, name: 'LEV', pos: 'MB', isLibero: false },
            { id: 'n4', number: 4, name: 'YAMAMOTO', pos: 'OH', isLibero: false },
            { id: 'n5', number: 6, name: 'FUKUNAGA', pos: 'OH', isLibero: false },
            { id: 'n6', number: 2, name: 'KAI', pos: 'OPP', isLibero: false }
        ],
        liberos: [{ id: 'nL1', number: 3, name: 'YAKU', pos: 'L', isLibero: true }],
        bench: [{ id: 'nb1', number: 7, name: 'INUOKA', pos: 'OH', isLibero: false }]
    }
};
...
const PlayerTag = ({ player, isSelected, teamColor, isLibero, isBlocker }) => {
    ...
    let borderClass = teamColor === 'orange' ? "border-orange-500" : "border-red-600";
    ...
    if (isLibero && !isSelected) {
        textClass = teamColor === 'orange' ? "text-orange-700" : "text-red-700";
        borderClass = teamColor === 'orange' ? "border-orange-600 ring-2 ring-inset ring-orange-200" : "border-red-600 ring-2 ring-inset ring-red-200";
    }

    if (isSelected) {
        if (teamColor === 'orange') {
            bgClass = "bg-gradient-to-r from-orange-500 to-orange-600 shadow-orange-200";
            textClass = "text-white";
            borderClass = "border-orange-800";
        } else {
            bgClass = "bg-gradient-to-r from-red-600 to-red-700 shadow-red-200";
            textClass = "text-white";
...
const CourtZone = ({ player, zoneNumber, isSelected, onClick, teamColor, isLibero, highlight, disabled, locked, isBlocker }) => {
    ...
    return (
        <div onClick={() => !disabled && !locked && onClick(player)} className={`w-full h-full border border-slate-200 flex flex-col items-center justify-center py-1 transition-colors gap-1 group relative ${containerClass} ${highlight ? 'bg-green-50 ring-4 ring-inset ring-green-400/50 opacity-100' : ''} ${isBlocker ? 'bg-purple-50 ring-4 ring-inset ring-purple-400/50 opacity-100' : ''}`}>
            ...
            {highlight && <div className="absolute top-1 right-1 text-[9px] font-bold text-green-700 bg-green-200 px-1 rounded z-30 animate-pulse">TARGET</div>}
            <div className="flex-grow flex items-center justify-center z-10 w-36 md:w-40 lg:w-44">
                <PlayerTag player={player} isSelected={isSelected} teamColor={teamColor} isLibero={isLibero} isBlocker={isBlocker} />
            </div>
        </div>
    );
};
...
const processRotation = (teamRotation, teamOriginals) => {
    ...
    const playerAtZ4 = newRotation[3];
    if (playerAtZ4.isLibero) {
        const originalPlayer = newOriginals[playerAtZ4.id];
        if (originalPlayer) {
            newRotation[3] = originalPlayer;
            delete newOriginals[playerAtZ4.id];
            autoSwapLog = { libero: playerAtZ4, original: originalPlayer };
        }
    }
    return { newRotation, newOriginals, autoSwapLog };
};

const reverseRotation = (teamRotation, teamOriginals) => {
    let newRotation = [...teamRotation];
    const mover = newRotation.shift();
    newRotation.push(mover);
    return { newRotation, teamOriginals };
};

// --- INITIAL STATE FACTORY (Consumes updated roster format) ---
const createInitialState = (rosters) => ({
    score: { home: 0, away: 0 },
    ...
    previousState: null,
    // Maps the nested object structure correctly
...
const gameReducer = (state, action) => {
    ...
    switch (action.type) {
        ...
        case 'REQUEST_TIMEOUT': {
            ...
            const teamCode = team === 'home' ? 'KAR' : 'NEK';
        ...
        case 'REFEREE_DECISION': {
            ...
            const teamCode = winner === 'home' ? 'KAR' : 'NEK';
            const snapshot = createSnapshot(state); // SAVE SNAPSHOT

            const { newRotation, newOriginals, autoSwapLog } = (winner !== state.servingTeam)
            ...
            if (autoSwapLog) l = [{ id: Date.now() + 1, time: getTime(), text: `ROTATION OUT`, type: 'info' }, ...l];
        ...
        case 'CHALLENGE_RESULT': {
            const { team, success, category } = action.payload;
            const challenger = team;
            const challengerCode = team === 'home' ? 'KAR' : 'NEK';

            if (!success) {
                const newChallenges = { ...state.challengesUsed };
                newChallenges[challenger] += 1;
                return { ...state, challengesUsed: newChallenges, logs: addLog(`CHALLENGE FAILED: ${category} by ${challengerCode}. Call Stands.`, 'danger') };
            }

            if (!state.previousState) return { ...state, logs: addLog(`CHALLENGE ERROR: No history.`, 'danger') };

            const restored = state.previousState;
            const newScore = { ...restored.score };
            newScore[challenger] += 1;

            let newRotations = { ...restored.rotations };
            let newOriginals = { ...restored.liberoOriginals };

            if (challenger !== restored.servingTeam) {
                const fwd = processRotation(restored.rotations[challenger], restored.liberoOriginals[challenger]);
                newRotations[challenger] = fwd.newRotation;
                newOriginals[challenger] = fwd.newOriginals;
            }

            return {
                ...state, score: newScore, servingTeam: challenger, rotations: newRotations, liberoOriginals: newOriginals,
                matchPhase: 'PRE_SERVE', previousState: null, selectedPlayer: null,
                rallyData: { serveType: null, serveResult: null, blockers: [], setter: null, attacker: null, receiver: null, target: null },
                logs: addLog(`CHALLENGE SUCCESS: ${category} -> POINT ${challengerCode} (Score Corrected)`, 'success')
            };
        }
        ...
        case 'EXECUTE_LIBERO_SWAP': {
            ...
            if (libero.isLibero) { newLRotation[zoneIndex] = libero; newLOriginals[libero.id] = courtPlayer; }
            else { newLRotation[zoneIndex] = libero; delete newLOriginals[courtPlayer.id]; }
            return { ...state, rotations: { ...state.rotations, [lTeam]: newLRotation }, liberoOriginals: { ...state.liberoOriginals, [lTeam]: newLOriginals }, matchPhase: 'PRE_SERVE', selectedPlayer: null, actionTeam: null, logs: addLog(`LIBERO SWAP: ${fmt(libero)} <-> ${fmt(courtPlayer)}`, 'info') };
        }

        // --- PLAYER SELECTION ---
        case 'SELECT_PLAYER': {
            const p = action.payload;
            const pTeam = p.id.startsWith('k') ? 'home' : 'away';

            if (state.matchPhase === 'SELECT_BLOCKERS') { const defTeam = state.possession === 'home' ? 'away' : 'home'; if (pTeam !== defTeam) return state; return gameReducer(state, { type: 'TOGGLE_BLOCKER', payload: p }); }
            if (state.matchPhase === 'COVER') { if (pTeam !== state.possession) return state; return gameReducer(state, { type: 'EXECUTE_COVER', payload: { ...p } }); }
            if (['RECEPTION', 'SET', 'ATTACK'].includes(state.matchPhase) && pTeam !== state.possession) return state;
            if (state.matchPhase === 'DIG_DECISION' || state.matchPhase === 'BLOCK_RESULT') return state;

            // --- SUBSTITUTION & LIBERO SWAP LOGIC ---
            if (state.matchPhase === 'SUBSTITUTION' || state.matchPhase === 'LIBERO_SWAP') {
                ...
                if (!state.selectedPlayer) {
                    ...
                    if (state.matchPhase === 'LIBERO_SWAP') return { ...state, selectedPlayer: p };
                    ...
                    if (state.matchPhase === 'LIBERO_SWAP') {
                        if (p1.isLibero && !p2.isLibero) {
                            const idx = state.rotations[pTeam].findIndex(r => r.id === p2.id);
                            if (idx === -1) return state;
                            if (idx > 2) { alert("Libero can only replace back-row players!"); return { ...state, selectedPlayer: null }; }
                            if (idx === 0 && pTeam === state.servingTeam) { alert("Libero cannot replace the server!"); return { ...state, selectedPlayer: null }; }
                            return gameReducer(state, { type: 'EXECUTE_LIBERO_SWAP', payload: { libero: p1, courtPlayer: p2, zoneIndex: idx, team: pTeam } });
                        }
                        if (p1.isLibero) {
                            const original = state.liberoOriginals[pTeam][p1.id];
                            if (original) {
                                const idx = state.rotations[pTeam].findIndex(r => r.id === p1.id);
                                return gameReducer(state, { type: 'EXECUTE_LIBERO_SWAP', payload: { libero: original, courtPlayer: p1, zoneIndex: idx, team: pTeam } });
                            }
                        }
                    }
                }
        ...
        case 'INIT_SERVE': if (state.matchPhase !== 'PRE_SERVE') return state; return { ...state, matchPhase: 'SERVE', lastServer: state.servingTeam, selectedPlayer: state.rotations[state.servingTeam][0], possession: state.servingTeam, rallyData: { serveType: null, serveResult: null, blockers: [], setter: null, attacker: null, receiver: null, target: null } };
...
export default function VolleyballTracker({ setNumber = 1, initialRosters = INITIAL_ROSTERS, onSetComplete }) {

    // Initialize state
    const [state, dispatch] = useReducer(gameReducer, null, () => createInitialState(initialRosters));
    ...
    useEffect(() => {
        ...
        if ((home >= targetScore || away >= targetScore) && Math.abs(home - away) >= 2) {
            const timer = setTimeout(() => {
                ...
                if (window.confirm(`Set Finished! Winner: ${winner === 'home' ? 'KARASUNO' : 'NEKOMA'}\nNext Set?`)) {
                    if (onSetComplete) onSetComplete(setSummary);
                }
            }, 500);
            return () => clearTimeout(timer);
        }
    }, [state.score, setNumber, onSetComplete, state.logs, state.timeoutsUsed, state.challengesUsed]);
    ...
    const renderCourtHalf = (side, rotation) => {
        ...
        else if (state.matchPhase === 'SUBSTITUTION' || state.matchPhase === 'LIBERO_SWAP') { if (state.actionTeam === teamName && state.selectedPlayer) isClickable = true; }
        ...
        const CardRow = ({ zones }) => (
            <div className="flex flex-col h-full gap-0.5">
                {zones.map(z => {
                    ...
                    if (state.matchPhase === 'LIBERO_SWAP' && state.selectedPlayer && state.actionTeam === teamName) {
                        const isServing = state.servingTeam === teamName;
                        const allowedZones = isServing ? [5, 6] : [1, 5, 6];
                        if (state.selectedPlayer.isLibero && !allowedZones.includes(z)) zoneDisabled = true;
                    }
                    ...
                    const isBlocker = state.rallyData.blockers.some(b => b.id === p.id);
        ...
        return isLeft ? (<div className="flex w-full h-full"><div className="flex-[2] h-full"><CardRow zones={[5, 6, 1]} /></div><div className="w-1 bg-orange-600/20 h-full relative z-10"></div><div className="flex-1 pl-4 h-full"><CardRow zones={[4, 3, 2]} /></div></div>) : (<div className="flex w-full h-full"><div className="flex-1 pr-4 h-full"><CardRow zones={[2, 3, 4]} /></div><div className="w-1 bg-red-600/20 h-full relative z-10"></div><div className="flex-[2] h-full"><CardRow zones={[1, 6, 5]} /></div></div>);
    };
        ...
        if (state.matchPhase === 'SUBSTITUTION' || state.matchPhase === 'LIBERO_SWAP') return (
            <div className="flex flex-col items-center justify-center h-full w-full gap-4 relative bg-slate-50 border-2 border-dashed border-slate-300 rounded-xl m-2">
                ...
                <div className="flex items-center gap-2 text-sm font-bold text-slate-500 bg-white shadow-sm border px-4 py-2 rounded-full">
                    ...
                    <ArrowRight size={14} />
                    <span>2. Select Court Player</span>
                </div>
                <div className="mt-2">
    ...
    const LiberoSection = ({ team, liberos, teamColor, rotation }) => (
        <div className="flex flex-col items-center w-full px-2 gap-2 mt-2">
            ...
            <div className="flex gap-2 w-full">
                <button onClick={() => dispatch({ type: 'REQUEST_LIBERO_SWAP', payload: team })} className={`flex-1 border-2 font-black text-[10px] py-1.5 rounded hover:bg-slate-50 transition-colors ${teamColor === 'orange' ? 'border-orange-100 text-orange-600' : 'border-red-100 text-red-600'}`}>
                    <RefreshCw size={14} className="mx-auto mb-0.5" /> SWAP
                </button>
            </div>
            <div className="flex flex-col gap-1 w-full">
                {liberos.map(lib => {
                    const isOnCourt = rotation.find(r => r.id === lib.id);
                    return isOnCourt
    ...
    return (
        <div className="w-full h-screen bg-slate-100 flex flex-col font-sans overflow-hidden text-slate-800">
            ...
            <header className="bg-slate-900 text-white shadow-lg shrink-0 z-40 h-16 flex relative overflow-hidden">
                <div className="absolute top-0 left-0 w-1/2 h-full bg-orange-600 skew-x-12 origin-bottom -translate-x-10 z-0 border-r-4 border-black"></div>
                <div className="absolute top-0 right-0 w-1/2 h-full bg-red-700 skew-x-12 origin-top translate-x-10 z-0"></div>
                <div className="w-32 xl:w-64 flex-shrink-0 flex items-center gap-4 pl-4 z-10"><div className="bg-black w-12 h-9 xl:w-16 xl:h-10 flex items-center justify-center rounded text-2xl xl:text-3xl text-orange-500 font-bold font-mono border-2 border-white">{state.score.home}</div></div>
                ...
                <div className="w-32 xl:w-64 flex-shrink-0 flex items-center justify-end gap-4 pr-4 z-10"><div className="bg-black w-12 h-9 xl:w-16 xl:h-10 flex items-center justify-center rounded text-2xl xl:text-3xl text-red-500 font-bold font-mono border-2 border-white">{state.score.away}</div></div>
            </header>
            ...
            <div className="flex-1 flex overflow-hidden relative">
                ...
                <div className="hidden md:flex flex-col w-32 xl:w-64 bg-white border-r border-slate-300 py-2 gap-2 z-20 shadow-xl flex-shrink-0">
                    <LiberoSection team="home" teamColor="orange" liberos={initialRosters.home.liberos} rotation={state.rotations.home} />
                    <div className="w-full border-t border-slate-100 my-1"></div>
                    <div className="px-2">
                        <button onClick={() => dispatch({ type: 'REQUEST_SUB', payload: 'home' })} disabled={['RALLY', 'ATTACK', 'SET', 'RECEPTION'].includes(state.matchPhase)} className="w-full bg-orange-50 border-2 border-orange-100 text-orange-600 font-black text-[10px] xl:text-xs py-2 rounded hover:bg-orange-100 disabled:opacity-50 flex items-center justify-center gap-2">
                ...
                <div className="hidden md:flex flex-col w-32 xl:w-64 bg-white border-l border-slate-300 py-2 gap-2 z-20 shadow-xl flex-shrink-0">
                    <LiberoSection team="away" teamColor="red" liberos={initialRosters.away.liberos} rotation={state.rotations.away} />
                    <div className="w-full border-t border-slate-100 my-1"></div>
                    <div className="px-2">
                        <button onClick={() => dispatch({ type: 'REQUEST_SUB', payload: 'away' })} disabled={['RALLY', 'ATTACK', 'SET', 'RECEPTION'].includes(state.matchPhase)} className="w-full bg-red-50 border-2 border-red-100 text-red-600 font-black text-[10px] xl:text-xs py-2 rounded hover:bg-red-100 disabled:opacity-50 flex items-center justify-center gap-2">
