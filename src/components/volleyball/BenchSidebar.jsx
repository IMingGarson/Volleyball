// src/components/volleyball/BenchSidebar.jsx
import { RefreshCw, Users } from 'lucide-react';
import { BenchCard, ChallengeButton, TimeoutButton } from '../common/ActionButtons';
import { PlayerTag } from '../common/PlayerTag';

// Helper component for Libero Section within Sidebar
const LiberoSection = ({ team, liberos, teamColor, rotation, onRequestSwap, onPlayerClick, selectedPlayerId }) => (
    <div className="flex flex-col items-center w-full px-2 gap-2 mt-2">
        ...
        <div className="flex gap-2 w-full">
            <button onClick={() => onRequestSwap(team)} className={`flex-1 border-2 font-black text-[10px] py-1.5 rounded hover:bg-slate-50 transition-colors ${teamColor === 'orange' ? 'border-orange-100 text-[#ff7b00]' : 'border-red-100 text-[#d9202a]'}`}>
        ...
        <div className="flex flex-col gap-1 w-full">
            {liberos.map(lib => {
                ...
                return isOnCourt
                    ...
                    : (<div key={lib.id} className="w-full" onClick={() => onPlayerClick(lib)}><PlayerTag player={lib} isLibero={true} teamColor={teamColor} isSelected={selectedPlayerId === lib.id} /></div>)
            })}
        </div>
    </div>
);
...
export default function BenchSidebar({ team, data, gameState, actions, onTimeout, onChallenge }) {
    ...
    const teamColor = isHome ? 'orange' : 'red';
    ...
    // Styling logic for border placement
        ...
        <div className={`hidden md:flex flex-col w-32 xl:w-64 bg-white ${borderClass} border-slate-300 py-2 gap-2 z-20 shadow-xl flex-shrink-0`}>
            ...
            <div className="px-2">
                <button
                    ...
                    className={`w-full bg-${teamColor}-50 border-2 border-${teamColor}-100 text-${teamColor === 'orange' ? '[#c75c14]' : '[#9e212d]'} font-black text-[10px] xl:text-xs py-2 rounded hover:bg-${teamColor}-100 disabled:opacity-50 flex items-center justify-center gap-2`}
                >
            ...
            <div className="px-2 pb-2 mt-auto flex flex-col gap-2">
                ...
                <ChallengeButton onClick={onChallenge} used={challengesUsed} limit={2} />
            </div>
        </div>
    );
}