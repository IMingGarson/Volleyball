...
export const gameReducer = (state, action) => {
    ...
    switch (action.type) {
        ...
        case 'REFEREE_DECISION': {
            ...
            // [CRASH FIX 1] Handle Replay first
            if (!winner || reason === 'Replay') {
                return {
                    ...
                    selectedPlayer: null,
                    rallyData: INITIAL_RALLY_DATA,
                    logs: addLog(`REFEREE: REPLAY POINT`, 'correction')
                };
            }

            // [CRASH FIX 2] Normalize Winner Input
            // The Ref Control might send 'A', 'B', 'KARASUNO', etc. We strictly need 'home' or 'away'.
            ...
            // Safety Check: Ensure the rotation exists before processing
            const targetRotation = state.rotations[winner];
            if (!targetRotation) {
                console.error("CRITICAL ERROR: Invalid winner team key:", winner);
                return state; // Prevent crash by ignoring invalid action
            }

            const { newRotation, newOriginals, autoSwapLog, returningPlayer } = shouldRotate
                ? processRotation(targetRotation, state.liberoOriginals[winner])
                : { newRotation: targetRotation, newOriginals: state.liberoOriginals[winner], returningPlayer: null };
            ...
            return {
                ...
                benches: newBenches, matchPhase: 'PRE_SERVE', logs: l, selectedPlayer: null,
                rallyData: INITIAL_RALLY_DATA
            };
        }

        // --- (Rest of the reducer remains unchanged) ---
        case 'CHALLENGE_RESULT': {
            ...
            if (autoSwapLog) l = [{ id: Date.now() + 1, time: getTime(), text: autoSwapLog.text, type: autoSwapLog.type }, ...l];
            return {
                ...baseState, score: { ...baseState.score, [winner]: baseState.score[winner] + 1 }, servingTeam: winner,
                rotations: { ...baseState.rotations, [winner]: newRotation }, liberoOriginals: { ...baseState.liberoOriginals, [winner]: newOriginals },
                benches: newBenches, matchPhase: 'PRE_SERVE', logs: l, rallyData: INITIAL_RALLY_DATA, previousState: createSnapshot(baseState), challengesUsed: state.challengesUsed
            };
        }
        ...
        case 'CONFIRM_BLOCK': { const { pendingResult } = state.rallyData; if (pendingResult === 'SHUTDOWN') { const defTeam = state.possession === 'home' ? 'away' : 'home'; const snapshot = createSnapshot(state); const shouldRotate = defTeam !== state.servingTeam; const { newRotation, newOriginals, returningPlayer } = shouldRotate ? processRotation(state.rotations[defTeam], state.liberoOriginals[defTeam]) : { newRotation: state.rotations[defTeam], newOriginals: state.liberoOriginals[defTeam], returningPlayer: null }; let newBenches = { ...state.benches }; if (returningPlayer) newBenches[defTeam] = newBenches[defTeam].filter(p => p.id !== returningPlayer.id); return { ...state, previousState: snapshot, score: { ...state.score, [defTeam]: state.score[defTeam] + 1 }, servingTeam: defTeam, rotations: { ...state.rotations, [defTeam]: newRotation }, liberoOriginals: { ...state.liberoOriginals, [defTeam]: newOriginals }, benches: newBenches, matchPhase: 'PRE_SERVE', logs: addLog(`SHUTDOWN POINT`, 'highlight'), selectedPlayer: null, rallyData: INITIAL_RALLY_DATA }; } return { ...state, matchPhase: 'BLOCK_RESULT', selectedPlayer: null }; }
        case 'EXECUTE_COVER': return { ...state, matchPhase: 'SET', selectedPlayer: null, logs: addLog(`COVER`, 'info') };
        case 'ATTACK_RESULT': { const { result } = action.payload; const attTeam = state.possession; const defTeam = attTeam === 'home' ? 'away' : 'home'; if (result === 'KILL') { const snapshot = createSnapshot(state); const shouldRotate = attTeam !== state.servingTeam; const { newRotation, newOriginals, returningPlayer } = shouldRotate ? processRotation(state.rotations[attTeam], state.liberoOriginals[attTeam]) : { newRotation: state.rotations[attTeam], newOriginals: state.liberoOriginals[attTeam], returningPlayer: null }; let newBenches = { ...state.benches }; if (returningPlayer) newBenches[attTeam] = newBenches[attTeam].filter(p => p.id !== returningPlayer.id); return { ...state, previousState: snapshot, score: { ...state.score, [attTeam]: state.score[attTeam] + 1 }, servingTeam: attTeam, rotations: { ...state.rotations, [attTeam]: newRotation }, liberoOriginals: { ...state.liberoOriginals, [attTeam]: newOriginals }, benches: newBenches, matchPhase: 'PRE_SERVE', logs: addLog(`KILL -> POINT`, 'success') }; } if (result === 'ERROR') { const snapshot = createSnapshot(state); const shouldRotate = defTeam !== state.servingTeam; const { newRotation, newOriginals, returningPlayer } = shouldRotate ? processRotation(state.rotations[defTeam], state.liberoOriginals[defTeam]) : { newRotation: state.rotations[defTeam], newOriginals: state.liberoOriginals[defTeam], returningPlayer: null }; let newBenches = { ...state.benches }; if (returningPlayer) newBenches[defTeam] = newBenches[defTeam].filter(p => p.id !== returningPlayer.id); return { ...state, previousState: snapshot, score: { ...state.score, [defTeam]: state.score[defTeam] + 1 }, servingTeam: defTeam, rotations: { ...state.rotations, [defTeam]: newRotation }, liberoOriginals: { ...state.liberoOriginals, [defTeam]: newOriginals }, benches: newBenches, matchPhase: 'PRE_SERVE', logs: addLog(`ATTACK ERROR`, 'danger') }; } if (result === 'DIG') return { ...state, matchPhase: 'SET', possession: defTeam, logs: addLog(`DIG`, 'info'), selectedPlayer: null }; return state; }
